...
test('Validate selected pet types from the list', async ({ page }) => {
  ...
  const rosyPetTypeEdition = page.locator('#type1')
  await expect(rosyPetTypeEdition).toHaveValue('cat')
  ...
  for (const pet of pets) {
    ...
    await expect(rosyPetTypeEdition).toHaveValue(pet)
  }
})
...
test('Validate the pet type update', async ({ page }) => {
  ...
  const rosyPetTypeEdition = page.locator('#type1')
  await expect(rosyPetTypeEdition).toHaveValue('dog')
  const dropdownMenu = page.getByRole('combobox')
  await dropdownMenu.selectOption('bird')
  await expect(rosyPetTypeEdition).toHaveValue('bird')
